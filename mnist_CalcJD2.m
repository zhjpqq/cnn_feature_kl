function mnist_CalcJD2(layerId,ep,epn,batchId,batchIdn)
% clear all;
% mnist_Main2(layerId,ep,epn,batchId,batchIdn);
% close all;
% set(0,'DefaultFigureWindowStyle','normal');
% 对单类样本计算收敛性曲线JD-Batch，分别为Train和Test载入前后2个batch的数据进行计算

% 导入数据
%  layerId = 1; ep = 1; epn = ep; batchId = 0;  batchIdn = batchId = 20;
set = {'Train','Test'}; 
layerIndex = layerId; 
epochP = ep;  epochN = epn;
batchIndexP = batchId;  batchIndexN = batchIdn; 
dataDir2 = 'E:\MatConvNet-1.0-beta17\examples\mnist\data\lenet-test';
expDir = 'mnist-lenet-exp';
