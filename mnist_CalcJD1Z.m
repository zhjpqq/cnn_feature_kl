function mnist_CalcJD1Z(ep,layerId)
% clear all;
% close all;
...
for classIdx = 1:classNum
    trainDensities{classIdx} = calcVectorsHistogramZ(trainData(:,trainLabel==classIdx),scaleTrain,binSize);
    testDensities{classIdx} = calcVectorsHistogramZ(testData(:,testLabel==classIdx),scaleTest,binSize);
end
